...
from random import random
                                                                               #
def get_movie_names(idx):
    '''Converts item idx to movie names list'''
    item_titles = pd.Series(items_df['title'].values, 
                            index=items_df['item_id']).to_dict()
    return list(map(item_titles.get, idx))
...
def compute_metrics(test, recs, top_N):
    ...
    test_recs['users_item_count'] = test_recs.groupby(
...
def stratify(train, filter_dur = 300, loyal_treshold = 15):
    ...
    counting = train_copy[train_copy['total_dur'] > filter_dur].groupby(
      level = 'user_id')['watched_pct'].count()
    counting = counting.to_frame(name = 'watched_items')
    counting['strata'] = (counting['watched_items'] >= 1) + 0
    counting['strata'] = (counting['watched_items'] >= loyal_treshold) + counting['strata']
    return counting['strata']

def get_strata_map(train, sample, filter_dur = 300, loyal_treshold = 15):
    strata_map = stratify(train, filter_dur, loyal_treshold)
    sample_users = sample['user_id'].unique()
    strata_missing = pd.Series(data = 0, index = sample_users)
    strata_map_full = strata_map.combine_first(strata_missing)
    return strata_map_full

def add_strata_info(train, sample, filter_dur = 300, loyal_treshold = 15):
    strata_map_full = get_strata_map(train, sample, filter_dur, loyal_treshold)
    ones = strata_map_full[strata_map_full == 1].index
    twos = strata_map_full[strata_map_full == 2].index
    sample_c = sample.copy()
    sample_c['strata'] = sample_c['user_id'].apply(
      lambda x: 1 if x in ones else 2 if x in twos else 0)
    #sample = sample_c.copy()
    return sample_c.loc[:, :]

def compute_metrics_stratified(train, test, recs, top_N, model_name, loyal_treshold = 15):
    if 'strata' not in train.columns:
        train = add_strata_info(train, train, loyal_treshold)
    if 'strata' not in test.columns:
        test = add_strata_info(train, test, loyal_treshold)
    stratas = sorted(test['strata'].unique())
    #stratas = [0, 1, 2]
    results = {}
    for strata in stratas:
        results[(model_name, strata)] = compute_metrics(
          test[test['strata'] == strata], recs, top_N)
    results[(model_name, 'all')] = compute_metrics(test, recs, top_N)
        ...
        for start, end in pairwise(date_range):
            ...
            if self.yield_known_items:
                ...
                if self.items_mapping != None:
                    items_mapped['item_id'] = items_mapped['item_id'].map(self.items_mapping.get)
                known_items = items_mapped[(items_mapped['consider_known'] == True) & 
                                           test_mask].groupby('user_id')['item_id'].apply(list).to_dict()
...
def generate_implicit_recs_mapper(
    ...
    def _recs_mapper(user):
        ...
        if return_scores:
            return [item_inv_mapping[item] for item, _ in recs], [score for _, score in recs]
        else:
            return [item_inv_mapping[item] for item, _ in recs]
    return _recs_mapper

class PopularRecommender():
    def __init__(self, max_K=10, days=30, item_column='item_id', dt_column='date', with_filter = False):
    ...
    def recommend_with_filter(self, train, user_ids, top_K = 10):  
        ...
        filter_func = lambda row: [item for item in row['total_recs'] 
                                   if item not in known_items[row['user_id']]][:top_K]
