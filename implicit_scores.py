...
overall_known_items = interactions_df.groupby('user_id')['item_id'].apply(list).to_dict()
...
interactions_df['order_from_recent'] = interactions_df.sort_values(
      by = ['last_watch_dt'], ascending = False).groupby('user_id').cumcount() + 1
warm_idx = np.intersect1d(interactions_df['user_id'].unique(), submission['user_id'].unique())
...
before_boosting_known_items = before_boosting.groupby('user_id')['item_id'].apply(list).to_dict()
before_boosting_known_items_mapped = {}
for user, recommend in before_boosting_known_items.items():
    before_boosting_known_items_mapped[user] = list(map(lambda x: items_mapping[x], recommend))
before_boosting['order_from_recent'] = before_boosting.sort_values(by = ['last_watch_dt'], ascending = False).groupby('user_id').cumcount() + 1
boost_warm_idx = np.intersect1d(before_boosting['user_id'].unique(), boosting_data['user_id'].unique())

# Getting implicit scores and saving to csv
impl_recs_for_submit =  get_implicit_candidates(interactions_df, overall_known_items_mapped, warm_idx)
impl_recs_for_submit.to_csv('data/impl_scores_for_submit.csv', index = False)

impl_recs_for_boost_train =  get_implicit_candidates(before_boosting, before_boosting_known_items_mapped, boost_warm_idx)
