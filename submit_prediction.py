...
def check_len_recs(recs):
    recs['len'] = recs['item_id'].apply(lambda x: len(x))
    print(f"{(recs['len'] > 10).sum()} юзеров имеют рекомендации длиной более 10")
    print(f"{(recs['len'] < 10).sum()} юзеров имеют рекомендации длиной менее 10")
    less_ten_ten = recs[recs['len'] < 10]['user_id']
    recs.drop('len', axis = 1, inplace = True)
    return less_ten_ten.values
...
def check_submission(recs, known_items = overall_known_items):
    """Проверка количества повторов рекомендаций в истории юзеров"""
    ...
    num_duplicated = 0
    #users_have_duplicates_in_recs = set()
    ...
    less_ten_ten = check_len_recs(recs)
    return less_ten_ten #users_have_duplicates_in_recs
...
warm_idx = np.intersect1d(submission['user_id'].unique(), interactions_df['user_id'].unique())
candidates = pd.read_csv('impl_scores_submit.csv', usecols = ['user_id', 'item_id', 'implicit_score'])
...
full_train = full_train.sort_values(by=['user_id', 'boost_pred'], ascending=[True, False])
...
idx_for_popular = list(set(submission['user_id'].unique()).difference(set(boost_recs['user_id'].unique())))
